...
//	for(j = 0; j < BUF_SIZE_WRITE; j++) {
//		cBufWrite[j] = j;
//	}
	// define what to send
	sscanf("++help", "%s", cBufWrite);
	cBufWrite[6] = 10; // \n
	cBufWrite[7] = 13; // \r
...
/*		if((ftStatus = FT_SetBaudRate(ftHandle[i], 9600)) != FT_OK) {
		...
		/* Write */
		if((ftStatus = FT_Write(ftHandle[i], cBufWrite, BUF_SIZE_WRITE, &dwBytesWritten)) != FT_OK) {
		//if((ftStatus = FT_Write(ftHandle[i], "++help", 6, &dwBytesWritten)) != FT_OK) {
 ...
 */
		...
		// read how many bytes there are to read
		ftStatus = FT_GetQueueStatus(ftHandle[i], &dwRxSize);
		
		
		if(ftStatus == FT_OK) {
			pcBufRead = (char*)realloc(pcBufRead, dwRxSize);
			memset(pcBufRead, 0x42, dwRxSize);

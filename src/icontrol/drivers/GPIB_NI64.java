 ...
 * undefined.<p>
 * 
 * This class is the successor of <code>GPIB_NI</code> which has been tested 
 * extensively. To be able to run Instrument Control on 64-bit Windows machines,
 * this class uses NI4882.dll instead of gpib-32.dll. The new dll is nominally
 * backwards compatible with miniman changes. This new version needs to be tested.<p>
 *
 * <h3>Usage:</h3>
 * <ul>
 *  <li>Instantiate the class. This calls <code>LoadNative</code> to establish
 *      access to the native code (for instance NI4882.dll on Windows OS).
 ...
 *
 */
public class GPIB_NI64 extends GPIB_Driver {

    ///////////////////
    // member variables

    /** Handle to access the native code */
    private static gpib64    m_gpib64 = null;

    /** The Logger for this class */
    private static final Logger m_Logger = Logger.getLogger("iC.Instruments.GPIB_NI64");
    ...
    // The new NI4882.dll defines methods to access the 
    // following global variables, and although specified in the
    // manual, does not export global variables any longer
    ...
    // <editor-fold defaultstate="collapsed" desc="ni4882.dll CONSTANTS">
    ...
    /** size of the Receive buffer. Must be smaller than largest int value */
     ...
     *
     * Please note that all function declaration have been imported from ni4882.h,
     ...
     * data type conversion is correct, and test it. Ensure that the "new" data
     * types in NI4882.dll are still the same (Functions taking in parameters that 
     * describe a pointer length now use size_t types. Status variables now use 
     * the unsigned long type.)
     */
    // <editor-fold defaultstate="collapsed" desc="gpib64 Interface Declarations">
    private interface gpib64 extends Library {
        ...
        //////////////////////////////////////////////
        // Function declaration (adapted from ni4882.h)
        
        // !!
        // !! check data types in ni4882.h when uncommenting commands !!
        // !!
...
//        void AllSpoll      (int boardID, short[] addrlist, ShortByReference results);//Addr4882_t * addrlist, PSHORT results
        ...
        void FindLstn      (int boardID, short[] addrlist, short[] results, long limit);//Addr4882_t * addrlist, Addr4882_t * results
                                                                            // was int for gpib-32.dll
...
//        void WaitSRQ       (int boardID, ShortByReference result);//PSHORT result
        ...
        /* Function to access process-wide GPIB global status variables. Only
         * Most likely only defined in NI4882.dll for Windows, not for Mac/Linux */
        long Ibsta();
        long Iberr();
        long Ibcnt();
...
//      #define ThreadIbcntl() ThreadIbcnt()
    ...
    protected GPIB_NI64() throws IOException {
     ...
     */
    // <editor-fold defaultstate="collapsed" desc="LoadNative">
    private void LoadNative()
        ...
        try {
            // load the native library
            m_gpib64 = (gpib64) Native.loadLibrary(LibraryName, gpib64.class);
            ...
            str += "On WinXP, ni4882.dll is typically installed in C:\\Windows\\System32.\n";
            str += "It might help to use the 32-bit NI drivers by adding GPIB_NI.Use64BitDriver=0 to <user home>/iC/iC_User.properties\n";
         ...
         *  With the new NI4882.dll, the global variables are encapsulated in 
         *  new methods, which are recommended to be used in new code. */
        try {
            // load variables on a Mac and under Linux
            if (!Platform.isWindows()) {
                // ibsta
                ...
                // in version 3.11 of NI488.2 the global status variables are not
                // exported, although promised in the manual and ni4882.h file.
...
//                m_ibcntl.setPointer(dummy);
    ...
    protected void Open(int GPIB_Address)
        ...
        if (USE_4882 == true) {
            ...
            // init device
            m_UnitDescriptor = m_gpib64.ibdev(BOARD_NR, GPIB_Address, 0,
            ...
            // clear device
            m_gpib64.ibclr(m_UnitDescriptor);
    ...
    protected long Send(String Msg)
        ...
        // send it
        if (USE_4882 == true) {
            m_gpib64.Send(BOARD_NR, (short)m_UnitDescriptor,
                    BBuffer, Msg.length(), EOT_MODE);
        } else {
            m_gpib64.ibwrt(m_UnitDescriptor, BBuffer, Msg.length());
        }
    ...
    protected String Receive(boolean Trim)
        ...
        // Receive the data
        if (USE_4882 == true) {
            m_gpib64.Receive(BOARD_NR, (short)m_UnitDescriptor, m_ReceiveBuffer,
                    m_ReceiveBuffer.limit(), RECEIVE_TERMINATION);
        } else {
            m_gpib64.ibrd(m_UnitDescriptor, m_ReceiveBuffer, m_ReceiveBuffer.limit());
        }
    ...
    public int ReadStatusByte()
        ...
        // read the status byte
        if (USE_4882 == true) {
            m_gpib64.ReadStatusByte(BOARD_NR, (short)m_UnitDescriptor, pShort);
            StatusByte = (int)pShort.getValue();
        } else {
            m_gpib64.ibrsp(m_UnitDescriptor, pByte);
            StatusByte = (int)pShort.getValue();
        }
     ...
     */
    // <editor-fold defaultstate="collapsed" desc="ClearAllDevices">
    private void ClearAllDevices()
        ...
        m_gpib64.FindLstn(BOARD_NR, LookUp, Results, 31);
        ...
        // devices on the bus.
        m_gpib64.DevClearList(BOARD_NR, Results);
     ...
     */
    // <editor-fold defaultstate="collapsed" desc="ClearDevice">
    private void ClearDevice()
        ...
        m_gpib64.FindLstn(BOARD_NR, LookUp, Results, 1);
        ...
        m_gpib64.DevClear(BOARD_NR, (short)m_UnitDescriptor);
    ...
    private void setTimeOut(int TimeOutConstant)
        ...
        m_gpib64.ibconfig(BOARD_NR, IbaTMO, TimeOutConstant);
     ...
     * error flag in the Status Byte <code>Ibsta</code> should, ideally, be
     ...
     */
    // <editor-fold defaultstate="collapsed" desc="checkErrorGPIB">
    ...
    protected void CloseInstrument() 
        ...
        if (USE_4882 == true) {
            ...
            m_gpib64.EnableLocal(BOARD_NR, dummy);
            
        } else {
            // second argument is 0
            m_gpib64.ibonl(m_UnitDescriptor, 0);
        }
    ...
    /* 
     * @return The global ibsta variable. On Windows machines, the corresponding
     * method in NI4882.dll is called, on other platforms, the corresponding
     * global variable is returned.
     */
    // <editor-fold defaultstate="collapsed" desc="Ibsta">
    long Ibsta() {
        
        if (Platform.isWindows()) {
            
            // use the method to return the status variable
            return m_gpib64.Ibsta();
        
        } else {
            
            // use the exported variable to report the status
            return m_ibsta.getValue();
        }
    }//</editor-fold>
    
    /* 
     * @return The global iberr variable. On Windows machines, the corresponding
     * method in NI4882.dll is called, on other platforms, the corresponding
     * global variable is returned.
     */
    // <editor-fold defaultstate="collapsed" desc="Iberr">
    long Iberr() {
        
        if (Platform.isWindows()) {
            
            // use the method to return the status variable
            return m_gpib64.Iberr();
        
        } else {
            
            // use the exported variable to report the status
            return m_iberr.getValue();
        }
    }//</editor-fold>
    
    /* 
     * @return The global ibcnt(l) variable. On Windows machines, the corresponding
     * method in NI4882.dll is called, on other platforms, the corresponding
     * global variable is returned.
     */
    // <editor-fold defaultstate="collapsed" desc="Ibcntl">
    long Ibcntl() {
        
        if (Platform.isWindows()) {
            
            // use the method to return the status variable
            return m_gpib64.Ibcnt();
